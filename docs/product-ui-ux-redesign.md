...
- 0.8.0：重构工作台任务创建与处理详情，加入最近项目、快速恢复、交付摘要、五阶段处理反馈和完成动作；高级质量与发布设置保持可访问但不干扰主流程。
- 0.8.1：统一单任务与批量任务的项目身份；“继续”直接恢复原项目并启动未完成阶段；批量断点重试的活动项目、阶段和进度同步至工作台。
- 0.9.0：增加批量文件预检与问题隔离；项目详情拆分为概览、处理记录和交付文件，可直接定位字幕及质量报告。
- 0.10.0：引入 Wpf.Ui 4.3.0 作为 Fluent 控件与图标基础；主窗口升级为 FluentWindow、接入现代标题栏和 Mica 背景，并以统一 Fluent 图标替换字符图标，工作台补回运行环境信息区。
- 0.11.0：按概念图重排工作台首屏，建立大尺寸视频拖放区、表格式最近任务、运行环境与真实磁盘空间卡；任务参数、进度和操作栏按上下文渐进展示。
- 0.12.0：按概念图重排批量任务页，加入真实概览统计、文件夹导入、重新预检、队列表格与右侧任务检查器。
- 0.13.0：建立统一 Fluent 组件基础层，统一按钮圆角和交互状态、键盘焦点、禁用态、输入控件规格，以及全局 DataGrid 表头、间距、交替行与选中状态。
- 0.14.0：重构项目库与项目详情，增加项目状态概览、即时搜索、状态筛选、项目进度列表、详情标签页，以及“无项目/无匹配结果”两类空状态。
- 0.15.0：重构字幕校订中心，升级为 FluentWindow，新增字幕/错误/提示/修改状态概览、问题筛选、问题导航、当前字幕编辑与固定保存发布操作栏。
- 0.16.0：新增主窗口内资源与模型管理页，将首次配置和日常维护分离，以独立状态卡管理 GPU、FFmpeg、Runtime、VAD、Whisper 模型和 DeepSeek。
- 0.17.0：将设置与诊断中心迁入主窗口页面；设置持久化任务默认值与交付策略，诊断页提供环境分组、自检、日志入口和本地生成的脱敏报告。
- 0.18.0：将项目库的字幕校订迁入主窗口，并按概念图落实字幕列表、视频预览、当前字幕检查器三栏工作区及固定交付操作栏。
- 0.18.1：依据实机截图校准校订中心的卡片拉伸、响应式视频尺寸、筛选工具栏、状态表达和播放器控制密度。
- 0.18.2：完善校订中心筛选统计、无投影字幕卡片、固定尺寸图标导航和双向时间微调交互。
- 0.19.0：建立颜色、度量、排版与组件样式分层的 Design Token 系统，并迁移主框架和字幕校订中心作为首批基准页面。
- 0.19.1：修正网格布局 Token 的 WPF 类型，确保设计系统在运行时正确加载。
- 0.20.0：将 Design Token 和产品组件样式扩展到工作台、批量任务、项目库、资源与模型、设置及诊断中心。
- 0.21.0：完成兼容层与硬编码颜色清理，修复筛选器组合状态，并加入 100%、125%、150% DPI 等效布局回归测试。
- 0.22.0：字幕校订中心加入可选 VLC 内嵌播放引擎和系统播放器自动降级，资源页提供 VLC 检测、路径选择与官方下载安装入口。
- 0.22.1：VLC 资源检测加入 PE 架构验证，32 位运行时会显示明确错误及 64 位安装路径提示。
- 0.22.2：关闭 VLC 自动字幕轨并建立播放时间与编辑字幕的双向同步，播放时列表、编辑器和视频覆盖字幕保持一致。
- 0.23.0：校订播放器升级为深色双层控制台，补齐拖动定位、音量、倍速、单句循环、快捷键及沉浸预览，并纳入多 DPI 布局回归。
- 0.23.1：统一 VLC 画布、视频画幅与控制栏宽度，消除宽窗口播放时的白色留边。
- 0.24.0：播放器按媒体原生画幅动态布局，并将字幕时间轴重构为带真实区间与播放头的紧凑编辑轨道。
- 0.25.0：工作台按产品设计稿重建任务入口、最近任务表格与逐项运行环境面板，并建立彩色设备徽标 Token。
- 0.25.1：精修最近任务表格、状态化操作、自定义进度组件、媒体元数据副标题与最大化窗口自适应高度。
- 0.26.0：项目库按高保真设计稿重构为项目总览、可筛选项目表、项目详情、处理流程和交付文件的一体化管理页。
- 0.27.0：项目列表加入真实视频帧缩略图与时长覆层，缩略图按源视频状态缓存并提供可靠回退。
- 0.28.0：收敛工作台、当前批次与项目库的职责边界，完成项由批次页确认结果、由项目库长期管理。
- 0.29.0：补齐批次生命周期，支持手动归档、历史结果查看以及从失败项或全部项目创建新批次。
- 0.30.0：工作台加入已选视频状态卡、最近活动缩略图以及批次、项目库、诊断中心的高频快捷入口。
- 下一步：进行全局高 DPI、键盘导航、长路径、空状态和窄窗口视觉验收，并建立 UI 回归清单。
