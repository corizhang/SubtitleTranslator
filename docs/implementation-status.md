...
## 2026-08-16 0.17.0 设置与诊断中心重构

- 设置与诊断入口由配置向导和消息框升级为主窗口内的独立页面，纳入统一导航与 Fluent 视觉体系。
- 设置页持久化默认字幕类型、源语言、最终质量检查和选择性翻译 QA，新任务启动时自动采用这些偏好。
- 字幕交付集中管理输出位置、命名策略、重名处理、自定义模板和输出目录，并保留实时目标路径预览。
- 本地数据区展示项目目录、占用和磁盘余量，并明确 DPAPI 密钥存储与字幕正文发送边界。
- 诊断中心按处理环境、硬件推理和日志支持分组，支持重新诊断与本地推理自检。
- 新增可复制、可导出的脱敏诊断报告，排除 API Key、视频路径、模型路径和字幕正文；应用日志单独提示可能包含本地路径。
- 用户设置结构向后兼容扩展，并新增默认任务偏好序列化往返覆盖。
- 应用及 MSI 版本提升为 `0.17.0`；自动化测试 52/52 通过。
...
## 2026-08-19 0.18.0 字幕校订中心高保真重构

- 项目库的“校订字幕”不再打开割裂的传统窗口，改为进入保留主导航的集成式校订页面。
- 页面按概念图重组为字幕检索与问题筛选、视频预览与时间轴、当前字幕检查器三栏布局。
- 字幕列表以卡片显示序号、时间、原文、译文和质量状态；支持按文本或序号搜索及仅看问题项。
- 当前字幕拆分原文与译文编辑，保留时间码修改、前后问题跳转、100 毫秒微调、重新检查和未保存离开提醒。
- 视频区使用本地 `MediaElement` 进行真实预览、播放与字幕叠加；系统编码能力不支持时明确降级到外部播放器。
- 底部操作栏集中显示保存状态，并提供保存、保存并重新发布，继续复用原有原子保存与备份机制。
- 应用及 MSI 版本提升为 `0.18.0`；自动化测试 52/52 通过。
## 2026-08-19 0.18.1 字幕校订中心截图校准

- 根据 2560×1400 实机截图修复字幕卡片宽度随内容变化的问题，列表项现在始终横向铺满并具有明确选中态。
- 搜索区增加图标与占位提示，质量筛选改为直接可见的快捷筛选按钮。
- 视频预览改为受工作区宽高共同约束的响应式 16:9 区域，避免大屏下无限占据纵向空间。
- 播放控制补充前后字幕导航与总时长，并压缩控制区和时间轴的视觉密度。
- 字幕卡片以绿、橙、红状态点取代重复的长问题文字，时间范围、原文和译文层级更接近产品概念图。
- 应用及 MSI 版本提升为 `0.18.1`。
## 2026-08-19 0.18.2 字幕校订控件细节优化

- 左侧搜索和筛选控件增高，四项筛选等宽铺满栏位，并展示全部、错误、建议和已修改的实时数量徽标。
- 移除冗余的“字幕列表”、上一问题和下一问题区域；字幕卡片序号与时间统一字号。
- 以无装饰的列表项模板消除卡片悬浮和选中时的组件库投影，仅保留边框及背景选中反馈。
- 右侧检查器缩窄，上一条和下一条改用固定 `38×38` 的 WPF UI `SymbolIcon` 按钮，并真正按相邻字幕切换。
- 时间输入框缩短；开始与结束时间均提供 `-100` 和 `+100` 双向微调，按钮文字缩小并用工具提示说明单位。
- 应用及 MSI 版本提升为 `0.18.2`。
## 2026-08-19 0.19.0 Design Token 基础系统

- 新增颜色、度量、排版和组件样式四层资源字典，`App.xaml` 仅负责按依赖顺序装配 WPF UI 与产品设计系统。
- 建立原始色板与语义画刷分层、4px 间距尺度、统一圆角、控件尺寸、页面布局尺寸和排版角色。
- 建立主按钮、次按钮、紧凑按钮、图标按钮、筛选按钮、输入框、下拉框、卡片、徽标和导航的产品组件样式。
- 保留旧资源键作为兼容别名，使尚未迁移的页面保持可用；新页面必须使用语义 Token。
...
## 2026-08-19 0.20.0 Design Token 全主页面迁移

- 工作台、批量任务、项目库、资源与模型、设置和诊断中心接入语义颜色、排版、页头、页脚和组件 Token。
- 新增统一的页面头部、页面底栏、摘要卡片、状态徽标、紧凑按钮和产品输入组件样式。
- 扩展中性图标、信息表面以及成功、警告、错误状态的语义画刷，业务页面不再用具体颜色表达状态含义。
- 高频页面的标题、关键指标、输入框、下拉框、状态徽标和检查器宽度统一由设计系统管理。
- 保留少量单页构图数值与旧 Token 兼容引用；后续在删除兼容层前继续进行机械清理和 DPI 视觉回归。
- 应用及 MSI 版本提升为 `0.20.0`。
## 2026-08-19 0.21.0 Design Token 收口与 DPI 回归

- 字幕校订筛选器改为完整 Token 化的 `ToggleButton`，默认、选中、悬浮、选中悬浮、按下和焦点状态由同一组件模板管理，修复选中后悬浮文字不可见。
- 四个筛选按钮统一等宽和外边距，Badge 保持各自业务状态色，选中态文字保持高对比度。
- 删除颜色、圆角和排版的旧兼容资源键，所有业务 XAML 改用语义 Token。
- 清理全部业务 XAML 的十六进制颜色；新增 `tools/Test-DesignTokens.ps1`，发布前自动拒绝硬编码颜色和旧资源键。
- 新增 Windows STA UI 布局测试项目，在 2560×1400 的 100%、125%、150% 等效逻辑视口实际 Measure/Arrange 字幕校订页面。
- DPI 回归验证筛选高度、四列等宽、中心编辑区最小宽度和底部操作栏可见性；测试曾发现并修复首尾按钮 3px 宽度差。
- 桌面截图工具仍因 `SetIsBorderRequired (0x80004002)` 无法捕获该 WPF 窗口，因此像素截图回归由用户实机验收补充，自动布局回归已纳入测试套件。
- 应用及 MSI 版本提升为 `0.21.0`。

## 2026-08-19 0.22.0 可选 VLC 内嵌播放引擎

- 应用仅引入轻量 `LibVLCSharp.WPF` 托管适配层，不在安装包中携带原生 LibVLC 或完整 VLC 桌面程序。
- “资源与模型”增加 VLC 播放引擎卡片，自动检测标准 64 位 VLC 安装目录，也允许用户选择包含 `libvlc.dll`、`libvlccore.dll` 和 `plugins` 的目录。
- VLC 是可选组件，不改变字幕生成所需 6 项资源的就绪判断；未安装时应用和首次配置向导保持正常工作。
- 字幕校订页优先使用已验证的 VLC 内嵌播放器，支持原文和译文覆盖层、播放暂停、字幕定位、时长和播放位置同步。
- VLC 初始化或播放失败时自动回退 WPF 系统播放器；系统播放器仍不支持时保留外部播放器入口。
- 播放器画面显示当前使用的播放引擎，运行时异常写入本地日志，便于诊断兼容性问题。
- 新增可选 VLC 目录完整性集成测试，设计 Token 审计与 100%、125%、150% DPI 等效布局测试继续通过。
- 应用及 MSI 版本提升为 `0.22.0`。

## 2026-08-19 0.22.1 VLC 架构诊断修复

- 修复资源页仅检查文件存在、把 `Program Files (x86)` 下的 32 位 VLC 错误标记为“已就绪”的问题。
- 诊断服务现在读取 `libvlc.dll` 的 PE 机器架构，只有 AMD64 运行时才能进入 x64 字幕编辑器。
- 若同时存在错误的手动目录和标准 64 位安装目录，自动优先选择完整的 64 位 VLC。
- 32 位、损坏或目录不完整时显示具体原因和正确的 `C:\Program Files\VideoLAN\VLC` 路径提示，不再等到播放时静默回退。
- 应用及 MSI 版本提升为 `0.22.1`。

## 2026-08-19 0.22.2 播放字幕同步

- VLC 实例和单个媒体源均关闭外挂字幕自动扫描，并在开始播放及计时更新时强制关闭 SPU 字幕轨，避免视频目录或 MKV 内封字幕与校订字幕重复显示。
- 播放计时器按当前时间匹配 `[开始, 结束)` 字幕区间，自动更新右侧编辑项、视频覆盖字幕和左侧列表选中项。
- 播放同步更新列表时抑制反向 Seek，避免“播放器选择字幕—SelectionChanged 又把播放器跳回开头”的循环。
- 字幕间隙自动隐藏覆盖字幕，暂停或手工选择字幕后恢复当前编辑字幕预览。
- 增加播放时间匹配边界和字幕间隙测试；应用及 MSI 版本提升为 `0.22.2`。

## 2026-08-20 0.25.1 最近任务与存储布局精修

- 最近任务表头提升字号和字重，并在表头上方增加独立分割线。
- FFprobe 媒体探测增加视频宽高输出；最近项目首次读取后缓存分辨率与时长，源文件变化时自动失效。
- 文件副标题统一显示“分辨率 丨 时长 丨 可用状态”。
- 新增独立 `TaskProgressIndicator` 组件，使用 10px 轨道并将百分比固定在右侧。
- 操作按钮按任务状态显示查看、查看进度、重试、重新开始或继续；已完成项目直接进入字幕校订。
- 存储卡片将项目目录与更改操作置于同一行，路径独占下一行并强制左对齐。
- 最近任务和存储卡片改为星号行布局，在最大化窗口中自动吸收剩余高度。
- 新增最大化工作台高度和任务状态动作语义回归测试。

## 2026-08-20 0.25.0 工作台产品化重构

- 工作台按设计稿重构为左侧主任务区与右侧状态栏，恢复清晰的页面层级和 2.45:1.1 栅格比例。
- 新建任务入口升级为大面积拖放区域，提供悬浮/拖入反馈、格式说明、选择视频和添加整季双入口。
- 最近任务升级为标准五列表格，分别展示任务、进度、状态、更新时间和恢复操作。
- 运行环境由三段摘要拆分为 GPU、FFmpeg、FFprobe、Whisper 模型、Whisper Runtime、Silero VAD、DeepSeek 和 VLC 独立状态行。
- 环境资源采用独立彩色矢量徽标与状态点，不依赖低 DPI 位图或手工切割雪碧图。
- 新增环境徽标语义色 Token，以及 1280×820 基准尺寸下的工作台栅格布局回归。

## 2026-08-20 0.24.0 原生画幅与字幕时间轴

- VLC 播放器解析实际视频轨的分辨率、像素宽高比和旋转方向；系统播放器读取自然视频尺寸。
- 播放器框支持 4:3、21:9、竖屏和旋转视频，并按照最终画幅动态居中缩放。
- 播放控件的响应式显示依据实际画面宽度，竖屏视频不会挤压或溢出控制栏。
- 字幕时间轴升级为紧凑的真实比例轨道，显示整片范围、当前字幕区间、片段时长和实时播放头。
- 时间轴内聚合前后 3 秒微调操作，移除原先占用大量空间的分散按钮布局。
- 新增 4:3、21:9、竖屏、像素宽高比和旋转元数据自动化回归测试。

## 2026-08-20 0.23.1 播放器画幅适配修正

- VLC 视频承载区域改用播放器深色背景，等比播放产生的留边不再显示为白色。
- 播放画面与控制栏按 16:9 画幅共同居中收缩，避免宽窗口下控制栏宽于实际视频。
- DPI 布局回归新增画面比例和画面/控制栏等宽断言。

## 2026-08-20 0.23.0 校订播放器重构

- 将原有表单式播放按钮重构为与视频画面连成一体的深色双层控制台：上层为全宽可交互进度条，下层集中放置播放与校订操作。
- 播放/暂停、上一条/下一条字幕、后退/前进 5 秒改用统一播放器图标和悬浮反馈；原时间轴的前后 3 秒按钮也接入真实 Seek。
- 进度条支持点击定位与拖动预览，拖动期间暂停计时器回写，释放后同时驱动 VLC 和 WPF 系统播放器。
- 增加音量滑杆、静音、0.5×–2.0×倍速控制，两种播放引擎使用同一套状态与操作。
- 增加“单句循环”，锁定启用时的字幕区间，在结束边界自动回到开始位置，方便反复核对听写和时间轴。
- 增加沉浸预览模式：隐藏页头、字幕列表、右侧检查器、字幕时间轴与底部发布栏，让播放器占满编辑工作区；Esc 或 F 可退出。
- 增加播放器快捷键：空格播放/暂停、左右方向键跳 5 秒、上下方向键切字幕、M 静音、L 单句循环、F 沉浸预览；文本编辑或下拉框聚焦时不会误触。
- 播放控件新增设计 Token 组件样式和尺寸 Token，并纳入 100%、125%、150% DPI 的高度、时间轴宽度和沉浸布局回归。
- 控制条增加窄屏响应式降级：按可用宽度隐藏低优先级的滑杆或辅助按钮，核心播放、Seek、静音和沉浸操作始终保留，1366×768 不发生横向叠加。
- 应用及 MSI 版本提升为 `0.23.0`。
## 2026-08-21 0.26.0 项目库高保真重构

- 项目库新增项目总数、已完成、可恢复、源文件异常四项实时统计卡片。
- 项目列表升级为搜索、状态筛选和五列信息表，补充分辨率、时长、进度、状态与更新时间。
- 项目详情按概览、处理记录、交付文件组织，概览展示媒体事实与七阶段处理流程。
- 底部操作根据任务状态显示“校订字幕”或恢复类动作，并集中提供重新发布、打开目录、清理缓存和删除项目。
- 媒体信息通过用户配置的 FFprobe 异步补全；新增项目库派生信息自动化测试。
- 应用及 MSI 版本提升为 `0.26.0`。
## 2026-08-21 0.26.1 项目库页脚绑定修复

- 移除 WPF `Run.Text` 对多个只读属性的绑定，改为单个只读摘要文本，避免内联对象重建时触发 TwoWay 回写异常。
- 安装器加入 Major Upgrade 规则，后续补丁版本能够可靠覆盖已安装版本。
- 应用及 MSI 版本提升为 `0.26.1`。
## 2026-08-21 0.27.0 项目视频缩略图

- 项目库使用已配置的 FFmpeg 从视频约 10% 处生成 320px 宽 JPEG 预览帧。
- 缩略图保存在项目缓存目录，并根据源视频修改时间自动复用或刷新。
- 首次生成限制为两个并发 FFmpeg 进程，避免大量历史项目同时占满 CPU、磁盘和 GPU。
- 图片使用内存解码，不锁定缓存文件；源文件缺失或生成失败时自动回退到视频图标。
- 应用及 MSI 版本提升为 `0.27.0`。
## 2026-08-21 0.27.1 项目详情栏精修

- 项目详情页头改为自适应高度，完整显示视频标题、路径、进度和项目占用。
- 移除与概览处理流程重复的“处理记录”页签，保留概览与交付文件。
- 七阶段流程改为连续轨道叠加状态节点，消除阶段之间的断线。
- 分辨率、时长、项目占用和源文件状态卡片增加统一语义图标。
- 应用及 MSI 版本提升为 `0.27.1`。
## 2026-08-21 0.28.0 任务信息架构收敛

- 工作台“最近任务”改为“最近活动”，按处理中、失败可恢复、暂停、未完成、已完成的关注优先级展示五项。
- 工作台只承担快速继续与校订入口，明确将完整历史、交付文件和缓存管理交给项目库。
- 批量任务改名为“当前批次”，完成项默认保留供本批次结果确认，并支持隐藏或清除已完成项。
- 批量状态统一为等待处理、正在处理、失败可重试、已取消和已完成；完成项明确提示项目已经保存。
- 批量完成项主操作改为“查看项目”，进入项目库继续校订、发布和长期管理。
- 应用及 MSI 版本提升为 `0.28.0`。
## 2026-08-21 0.29.0 历史批次与归档

- 当前队列加入稳定批次 ID、可编辑名称和创建时间，旧版 `batch-queue.json` 自动补全新字段。
- 新增独立 `batch-history.json`，历史记录只保存执行快照和项目关联路径，不复制视频、字幕或项目缓存。
- 支持手动“归档并新建”；存在失败、取消或预检问题时先提示确认，运行中的批次不可归档。
- 批量任务页面增加“当前批次/历史批次”页签，历史页展示批次结果摘要和逐项只读执行结果。
- 支持使用历史批次全部项目或仅失败/取消项创建新批次；非空当前批次不会被静默覆盖。
- 删除历史批次只删除执行记录，不影响项目库、字幕文件和原视频。
- 应用及 MSI 版本提升为 `0.29.0`。
## 2026-08-21 0.30.0 工作台状态化入口

- 新建任务区拆分为空状态与已选视频状态；选择视频后显示文件名、路径、大小和识别到的音轨摘要。
- 已选视频状态提供“更换视频”和“清除选择”，不再继续展示拖放空状态与单独路径文本。
- 最近活动复用项目库的 FFmpeg 缩略图缓存和时长覆层，两个页面的视频视觉标识保持一致。
- 存储卡扩展为“存储与快捷入口”，增加当前批次、项目库状态摘要、诊断中心和日志入口。
- 快捷入口只承担导航，批次执行、长期项目管理和诊断仍留在各自专属页面。
- 应用及 MSI 版本提升为 `0.30.0`。
